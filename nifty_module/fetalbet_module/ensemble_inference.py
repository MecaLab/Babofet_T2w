...
def run_majority_voting_pipeline(image_path, output_file_path):
    ...
    # Utilisation d'un chemin absolu pour éviter les problèmes de répertoire courant
    temp_dir = Path(os.path.abspath(f"tmp_{uuid.uuid4().hex}"))
    ...
    try:
        ...
        for model in model_paths:
            ...
            ], check=True)  # check=True fait planter proprement le subprocess s'il y a une erreur
