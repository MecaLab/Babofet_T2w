...
def dilate_bm_bis(input_file, output_file):
    ...
    command = f"fslmaths {input_file} -dilM -dilM -dilM -dilM {output_file}"
    subprocess.run(command, shell=True)

    command = f"fslmaths {output_file} -uthr 2 {output_file}"
    subprocess.run(command, shell=True)
...
if __name__ == "__main__":
    ...
    for ts in atlas_timepoints:
        ...
        if not os.path.exists(filename_in):
            ...
            continue
        filename_out = os.path.join(seg_folder, "tmp", f"ONPRC_G{ts}_NFseg_3_uthr2.nii.gz")
