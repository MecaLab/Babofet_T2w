...
view_mode = "axial"  # Modifie ici pour changer de vue
...
for i, idx in enumerate(idxs):
    if view_mode == "axial":
        slice_vol1 = vol1_data[:, :, idx]
        slice_vol2 = vol2_data[:, :, idx]
        mask_vol1 = brainmask1[:, :, idx]
        mask_vol2 = brainmask2[:, :, idx]

        line_pos = vol1_data.shape[0] // 2  # Ligne au milieu
        ...
        slice_vol1 = vol1_data[:, vol1_data.shape[1] // 2, :]
        slice_vol2 = vol2_data[:, vol2_data.shape[1] // 2, :]
        mask_vol1 = brainmask1[:, vol1_data.shape[1] // 2, :]
        mask_vol2 = brainmask2[:, vol2_data.shape[1] // 2, :]

        line_pos = vol1_data.shape[0] // 2
        intensity_profile_vol1 = slice_vol1[line_pos, :] * mask_vol1[line_pos, :]
        intensity_profile_vol2 = slice_vol2[line_pos, :] * mask_vol2[line_pos, :]

    elif view_mode == "coronal":
        slice_vol1 = vol1_data[vol1_data.shape[0] // 2, :, :]
        slice_vol2 = vol2_data[vol2_data.shape[0] // 2, :, :]
        mask_vol1 = brainmask1[vol1_data.shape[0] // 2, :, :]
        mask_vol2 = brainmask2[vol2_data.shape[0] // 2, :, :]

        line_pos = vol1_data.shape[1] // 2
        intensity_profile_vol1 = slice_vol1[:, line_pos] * mask_vol1[:, line_pos]
        intensity_profile_vol2 = slice_vol2[:, line_pos] * mask_vol2[:, line_pos]
    ...
    axes[i, 0].plot([0, slice_vol1.shape[0]-1], [line_pos, line_pos], color='red', linewidth=2)

    axes[i, 1].imshow(slice_vol2.T, cmap='gray', origin='lower')
    axes[i, 1].set_title(f'Vol 2 - {view_mode.capitalize()} at idx={idx}')
    axes[i, 1].plot([0, slice_vol2.shape[0]-1], [line_pos, line_pos], color='red', linewidth=2)
