...
for session in sessions:
    ...
    for i, axial_index in enumerate(axial_indices):
        # Obtenir les limites de la ROI pour la slice sagittale
        mask_slice = mask_data1[:, mid_sagittal_index1, :]
        x_min, x_max, y_min, y_max = get_roi_bounds(mask_slice)

        # Volume 1
        plt.subplot(len(axial_indices), 3, 3 * i + 1)
        sagittal_slice1 = data1[x_min:x_max, mid_sagittal_index1, y_min:y_max]
        plt.imshow(sagittal_slice1.T, cmap='gray', origin='lower')
        plt.axhline(y=axial_index - y_min, color='r', linestyle='--')
        plt.title(f'Volume 1 - Sagittal Slice {mid_sagittal_index1}, Axial Index {axial_index}')

        # Volume 2
        plt.subplot(len(axial_indices), 3, 3 * i + 2)
        sagittal_slice2 = data2[x_min:x_max, mid_sagittal_index1, y_min:y_max]
        ...
        intensity_profile1 = data1[x_min:x_max, mid_sagittal_index1, axial_index]
        intensity_profile2 = data2[x_min:x_max, mid_sagittal_index1, axial_index]
