...
def qc_recons_bis(base_path, subject, mode):
    ...
    for session in os.listdir(base_path):
        ...
        if exp_param_folder:
            anat_path = os.path.join(session_path, mode_folder, "exp_param", f"{subj_name}_haste_3DHR_{mode}_bm_DR5_pipeline.nii.gz")
            bm_path = os.path.join(session_path, mode_folder, "exp_param", f"{subj_name}_haste_3DHR_{mode}_bm_DR5_pipeline_mask.nii.gz")
        else:
        ...
        filename_out = os.path.join(mid_dir_snapshots, f"{subj_name}_{mode}_DR5_recons.png")
